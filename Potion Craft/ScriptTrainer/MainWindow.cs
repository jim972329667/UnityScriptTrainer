    ...
    public class MainWindow : MonoBehaviour
    {
        ...
        public static bool optionToggle
        {
            ...
            set
            {
                ...
                if (_optionToggle)
                {
                    ...
                    Managers.Cursor.gameObject.SetActive(false);
                    Cursor.visible = true;
                }
                else//关闭菜单
                {
                    dragAndDrog.isMouseDrag = false;
                    Managers.Cursor.gameObject.SetActive(true);
        ...
        private void CreateUI()
        {
            if (canvas == null)
            {
                ...
                #region[创建标题 和 关闭按钮]
                AddTitle(ScriptTrainer.Instance.Info.Metadata.Name + " by:Jim97 版本:" + ScriptTrainer.Instance.Info.Metadata.Version, background);
                ...
                AddH3("常用功能：", BasicScripts);
                {
                    ...
                    {
                        UIWindows.SpawnInputDialog($"您想获得多少个金币？", "获得", "100", (string count) =>
                        {
                            Scripts.AddGold(count.ConvertToIntDef(100));
                        });
                    });
                    AddButton("增加经验", BasicScripts, () =>
                    {
                        UIWindows.SpawnInputDialog($"您想获得多少经验？", "获得", "100", (string count) =>
                        {
                            Scripts.AddExperience(count.ConvertToIntDef(100));
                        });
                    });
                    AddButton("增加人气", BasicScripts, () =>
                    {
                        UIWindows.SpawnInputDialog($"您想获得多少人气？", "获得", "100", (string count) =>
                        {
                            Scripts.AddPopularity(count.ConvertToIntDef(100));
                        });
                    });
                    AddButton("增加声望", BasicScripts, () =>
                    {
                        UIWindows.SpawnInputDialog($"您想获得多少声望？", "获得", "100", (string count) =>
                        {
                            Scripts.AddKarma(count.ConvertToIntDef(100));
                        });
                    });
                    AddButton("增加技能点", BasicScripts, () =>
                    {
                        UIWindows.SpawnInputDialog($"您想获得多少技能点？", "获得", "5", (string count) =>
                        {

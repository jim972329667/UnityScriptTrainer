...
namespace ScriptTrainer
{
    internal class ItemWindow : MonoBehaviour
    {
        ...
        //搜索过滤
        private static List<ItemDefinition> FilterItemData(List<ItemDefinition> dataList)
        {
            if (searchText == "")
            {
                return dataList;
        ...
        private static string GetItemName(ItemDefinition item)
        {
        ...
        private static string GetItemDescription(ItemDefinition item)
        {
            return ItemDescription.GetDescription(item, 0, 0, false, false, true, false, false);
        }
        private static Sprite GetItemIcon(ItemDefinition item)
        {
            var x = new UninteractableRewardItemBhv();
            x.Init(item);
            return  x.im
        }
        #endregion

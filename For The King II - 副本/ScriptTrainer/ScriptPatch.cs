...
namespace ScriptTrainer
{
    public class ScriptPatch
    {
        ...
        [HarmonyPatch(typeof(PartyManagementDirector), "_showLoadOutParty")]
        public class PartyManagementDirectorOverridePatch__showLoadOutParty
        {
            [HarmonyPostfix]
            public static void Postfix(PartyManagementDirector __instance)
            {
                if (InfiniteLoadoutPoints)
                {
                    Traverse.Create(__instance).Field("_baseLoadOutPoints").SetValue(999);
                }
            }
        }

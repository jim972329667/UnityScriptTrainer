    ...
    public class MainWindow : MonoBehaviour
    {
        ...
        private void CreateUI()
        {
            if (canvas == null)
            {
                ...
                #region[创建标题 和 关闭按钮]
                AddTitle($"{ScriptTrainer.Title} by:Jim97");
                ...
                {
                    AddButton("添加金币", BasicScripts, () =>
                    {
                        UIWindows.SpawnInputDialog($"您想添加多少金币？", "添加", "100", (string count) =>
                        {
                            Scripts.DropCoin(count.ConvertToIntDef(100));
                        });
                    });
                    AddButton("恢复状态", BasicScripts, () =>
                    {
                        Scripts.GetRecovery();
                    });
                    ...
                    //AddToggle("无限起步点数",150, BasicScripts, (bool state) =>
                    //{
                    //    Scripts.InfinitePreparationPoints(state);
                    //});
                    hr();

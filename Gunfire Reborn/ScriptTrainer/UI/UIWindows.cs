...
namespace ScriptTrainer.UI
{
    internal class UIWindows
    {
        public static void MessageDialog(string prompt)
        {
            ...
            // 创建确定按钮
            GameObject uiButton = UIControls.createUIButton(uiPanel, "#8C9EFFFF", "确认", () =>
            {
        ...
        public static void SpawnInputDialog(string title, string button, string defaultText, Action<string> onFinish)
        {
            GameObject canvas = UIControls.createUICanvas();    // 创建画布
            Object.DontDestroyOnLoad(canvas);
            // 设置置顶显示
            ...
            Sprite txtBgSprite = UIControls.createSpriteFrmTexture(UIControls.createDefaultTexture("#7AB900FF"));
            GameObject uiText = UIControls.createUIText(uiPanel, txtBgSprite, "#FFFFFFFF");
            uiText.GetComponent<Text>().text = title;
            uiText.GetComponent<RectTransform>().localPosition = new Vector3(0, 10, 0);
            ...
            // 创建确定按钮
            GameObject uiButton = UIControls.createUIButton(uiPanel, "#8C9EFFFF", button, () =>
            {

        ...
        private void BasicScriptsTable(Rect TableRect)
        {
            ...
            GUILayout.BeginArea(TableRect);
            {
                scrollPosition = GUILayout.BeginScrollView(scrollPosition, false, false, GUILayout.Width(700), GUILayout.Height(300));
                {
                    ...
                    {
                        {
                            if (XmGUI.Button("1111"))
                            {
                                //foreach(Appliance item in GameData.Main.Get<Appliance>())
                                //{
                                //    Debug.Log($"ZG;{item.Name};{item.ID}");
                                //}

                                foreach(var x in GameData.Main.Prefabs)
                                {
                                    Debug.Log($"ZG;{x.Value}");
                                }
                            }
                            //SetUpItems items = new SetUpItems();

                            //World createdWorld = new WorldBootstrapper("Game", GameConnectionMode.All).CreatedWorld;
                            //Persistence.AutoLoad<PackProgressionSaveSystem>(createdWorld.EntityManager);

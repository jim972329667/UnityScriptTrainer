...
namespace ScriptTrainer
{
    internal class ItemWindow : MonoBehaviour
    {
        ...
        private static GameObject CreateItemButton(string ButtonText, Card Item, GameObject panel, UnityAction action)
        {
            ...
            tip.Init(GetItemDescription(Item), Item);
        ...
        private static Sprite GetItemIcon(Card item)
        {
            ...
            {
                return (Sprite)Game.Get().EssentialImageAsset.AddImage(item.ImageAssetPath, item.ImageAssetPath, AssetLifeSpan.PLAY_THROUGH);
            }
            else
            {
                return CardAssets.GetImageSprite(item.GetCurrentCardContent());
            }
        }
        #endregion
    }

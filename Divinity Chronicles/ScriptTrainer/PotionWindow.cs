...
{
    internal class PotionWindow : MonoBehaviour
    {
        ...
        public PotionWindow(GameObject panel, int x, int y)
        {
            Panel = panel;
        ...
        private void SearchBar(GameObject panel)
        {
            ...
            uiInputField.GetComponent<InputField>().onEndEdit.AddListener((string text) =>
            {
                ...
                if (TryGetData())
                    container();
                PotionWindow.uiText.GetComponent<Text>().text = uiText_text;
                //Destroy(ItemPanel);
            });
        }
        //分页
        ...
        private static void container()
        {
            ...
            int num = 0;
            foreach (Potion item in GetItemData())
            {
        ...
        private static GameObject CreateItemButton(string ButtonText, Potion Item, GameObject panel, UnityAction action)
        {
            //按钮宽 200 高 50
            int buttonWidth = 190;
            int buttonHeight = 50;
        ...
        public static void SpawnItem(Potion item)
        {
            Game.Get().GetPlayer().PotionContainer.AddPotion(item);
        }
        ...
        private static List<Potion> GetItemData()
        {

            List<Potion> ItemData = new List<Potion>();

            List<Potion> Potions = Game.Get().GetPlayer().PotionPool.GetPotions();
        ...
        //搜索过滤
        private static List<Potion> FilterItemData(List<Potion> dataList)
        {
            if (searchText == "")
            {
                return dataList;
        ...
        private static string GetItemName(Potion item)
        {
            return item.Name;
        }
        private static string GetItemDescription(Potion item)
        {
            return item.GetDescription();
        }
        private static Sprite GetItemIcon(Potion item)
        {
            return GameAssetCache.Get().GetPotionSprite(item);
        }

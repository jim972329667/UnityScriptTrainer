...
{
    internal class RelicWindow : MonoBehaviour
    {
        ...
        public RelicWindow(GameObject panel, int x, int y)
        {
            Panel = panel;
        ...
        private void SearchBar(GameObject panel)
        {
            ...
            uiInputField.GetComponent<InputField>().onEndEdit.AddListener((string text) =>
            {
                ...
                if (TryGetData())
                    container();
                RelicWindow.uiText.GetComponent<Text>().text = uiText_text;
                //Destroy(ItemPanel);
            });
        }
        //分页
        ...
        public static void SpawnItem(Relic item)
        {
            Game.Get().GetPlayer().AddRelic(item, null);
        }
        ...
        private static List<Relic> GetItemData()
        {
            ...
            List<Relic> ItemData = new List<Relic>();

            Dictionary<string, Relic> relics = Game.Get().GetPlayer().RelicPool.GetRelics();
            foreach (var relic in relics)
            {
                if(!GetItemName(relic.Value).StartsWith("Relic/IDS"))
                    ItemData.Add(relic.Value);
            }
        ...
        {
            var m_relicInMerchantcompanionSetInLocation = Resources.Load<GameObject>("CombatObjects/RelicUIInMerchant");
            LocMerchant.RelicInfo relicInfo = new LocMerchant.RelicInfo()
            {
                Relic = item,
                EnoughGold = true,
                Price= 0,
            };
            GameObject gameObject = Object.Instantiate<GameObject>(m_relicInMerchantcompanionSetInLocation);

            RelicInMerchant component3 = gameObject.GetComponent<RelicInMerchant>();
            component3.SetRelic(relicInfo);

            string text = "Images/Relics/" + item.OriginalName.ToLower().Replace(" ", "_");
            ResourceBankSprite component = gameObject.GetComponent<ResourceBankSprite>();
            if (component)
            {
                component.SpriteKey = text;
                return component.spriteData;
            }
            else
            {
                Debug.LogError("NULL Sprite Bank");
                return null;
            }

    ...
    public class MainWindow : MonoBehaviour
    {
        ...
        private void CreateUI()
        {
            if (canvas == null)
            {
                ...
                #region[添加功能按钮]
                BasicScripts.AddH3(ref Position, "基础修改：");
                {
                    BasicScripts.AddButton(ref Position, "添加现金流", () =>
                    {
                        UIWindows.SpawnInputDialog($"您想添加多少现金流？", "添加", "10000", (string count) =>
                        {
                            SchoolModule.instance.AddMoney(count.ConvertToIntDef(10000), MoneyUseType.other, null);
                           
                        });
                    });
                    BasicScripts.AddButton(ref Position, "添加特性点", () =>
                    {
                        foreach (var script in GameModeModule.instance.CurGameModeConfig.initialFurnitureId)
                        {
                            Debug.Log(script.GroupName.GetText());

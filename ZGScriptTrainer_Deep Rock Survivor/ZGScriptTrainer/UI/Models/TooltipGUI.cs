...
namespace ZGScriptTrainer.UI.Models
{
    public class TooltipGUI : MonoBehaviour
    {
        ...
        private GameObject BasePanel => ZGUIManager.UIToolTip;
        private RectTransform ParentPanel;
        public void Initialize(string text, GameObject panel, Color backgroundColor = default, Color textColor = default)
        {
            ParentPanel = panel.GetComponent<RectTransform>();
            Tooltip = text;
            Background = UIFactory.CreateUIObject("GridUIRoot", BasePanel, new Vector2(200, 60));
            ...
            TooltipText.GetComponent<RectTransform>().sizeDelta = new Vector2(150, size);
            ZGScriptTrainer.WriteLog($"size : {size + 10}");

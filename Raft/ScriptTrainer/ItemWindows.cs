...
namespace ScriptTrainer
{
    internal class ItemWindow : MonoBehaviour
    {
        ...
        private static void container()
        {
            ...
            foreach (Item_Base item in GetItemData())
            {
                if (num % 3 == 0)
                {
                    //if(num != 0)
        ...
        private static GameObject CreateItemButton(string ButtonText, Item_Base item, GameObject panel, UnityAction action)
        {
            //按钮宽 200 高 50
            int buttonWidth = 190;
            int buttonHeight = 50;
        ...
        public static void SpawnItem(Item_Base item, int amount)
        {
            var player = ComponentManager<Network_Player>.Value;
            player.Inventory.AddItem(item.UniqueName, amount);
        }
        ...
        {
            if (ScriptPatch.allAvailableItems.Count == 0)
            {
                return false;
            }
            return true;
        }
        private static List<Item_Base> GetItemData()
        {

            List<Item_Base> ItemData = ScriptPatch.allAvailableItems;
            ...
            //Curse诅咒 Magazine
            //对 DataList 进行分页
            List<Item_Base> list = new List<Item_Base>();
        ...
        //搜索过滤
        private static List<Item_Base> FilterItemData(List<Item_Base> dataList)
        {
            if (searchText == "")
            {
                return dataList;
        ...
        private static string GetItemName(Item_Base item)
        {
            return item.settings_Inventory.DisplayName;
        }
        private static string GetItemDescription(Item_Base item)
        {
            return item.settings_Inventory.Description;
        }
        private static Sprite GetItemIcon(Item_Base item)
        {

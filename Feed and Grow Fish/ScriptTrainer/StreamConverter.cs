...
namespace ScriptTrainer
{
    ...
    internal static class StreamConverter
    {
        ...
        private static Texture2D LoadTexture(this Stream stream, bool isLinear = false)
        {
            ...
            Texture2D texture2D = new Texture2D(2, 2, TextureFormat.RGBA32, false, isLinear);
            texture2D.LoadImage(data);
            return texture2D;
        }
        public static GameSkin ReadSkinFile(this string file)
        {
            ScriptTrainer.Instance.Log("加载文件00");
            try
            {
                ScriptTrainer.Instance.Log("加载文件0");
                if (File.Exists(file))
                {
                    ScriptTrainer.Instance.Log("加载文件1");
                    FileStream dataStream;
                    dataStream = new FileStream(file, FileMode.Open);
                    ScriptTrainer.Instance.Log("加载文件2");
                    Aes oAes = Aes.Create();
                    ...
                    ScriptTrainer.Instance.Log("解密文件");
                    ...
                    using (var zipArchive = new ZipArchive(decryptedStream, ZipArchiveMode.Read, true))
                    {
                        foreach (var entry in zipArchive.Entries)
                        {
                            ...
                            {
                                ScriptTrainer.Instance.Log("加载.preset");

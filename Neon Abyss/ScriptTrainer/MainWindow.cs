    ...
    public class MainWindow : MonoBehaviour
    {
        ...
        public static bool optionToggle
        {
            ...
            set
            {
                ...
                if (_optionToggle)
                {
                    ...
                    CursorVisible = Global.ForceHideCursor;
                    FreezeInput = Global.GetGameMode<ControlPlayerMode>().Player.FreezeInput;
                    Global.GetGameMode<ControlPlayerMode>().Player.FreezeInput = true;
                ...
                else
                {
                    ...
                    Global.GetGameMode<ControlPlayerMode>().Player.FreezeInput = FreezeInput;
                    Global.ForceHideCursor = CursorVisible;
                    Time.timeScale = 1f;
                }
                if (!initialized)
                {
        ...
        private void CreateUI()
        {
            if (canvas == null)
            {
                ...
                #region[创建标题 和 关闭按钮]
                UIWindows.AddTitle(uiPanel, $"{ScriptTrainer.Instance.Info.Metadata.Name} by:Jim97");
                ...
                //BasicScripts.GetComponent<Image>().color = UIControls.HTMLString2Color("#000000FF");
                ...
                {
                    UIWindows.AddButton(ref point, "增加1000金币", BasicScripts,  () =>
                    {
                        //CurrencyManager.Instance.AddGold(1000);
                    });
                    //PickInput = UIWindows.AddInputField(ref point, 120, ScriptTrainer.PickMult.Value.ToString(), BasicScripts, (string text) =>
                    //{
                    //    ScriptTrainer.PickMult.Value = text.ConvertToIntDef(2);
                    //});
                    //UIWindows.AddToggle(ref point, "多倍堆叠", BasicScripts, ScriptTrainer.ChangeStackSize, (bool state) =>
                    //{
                    //    StackInput?.SetActive(!state);
                    //});

...
namespace ScriptTrainer
{
    internal class ItemWindow : MonoBehaviour
    {
        ...
        private static void container()
        {
            ...
            foreach (Loot item in GetItemData())
            {
                if (num % 3 == 0)
                {
                    //if(num != 0)
        ...
        private static GameObject CreateItemButton(string ButtonText, Loot item, GameObject panel, UnityAction action)
        {
            //按钮宽 200 高 50
            int buttonWidth = 190;
            int buttonHeight = 50;
        ...
        public static void SpawnItem(Loot item)
        {
        ...
        private static List<Loot> GetItemData()
        {
            ...
            List<Loot> ItemData = dataAsset.data.ToList<Loot>();
        ...
        //搜索过滤
        private static List<Loot> FilterItemData(List<Loot> dataList)
        {
            if (searchText == "")
            {
                return dataList;
        ...
        private static string GetItemName(Loot item)
        {
            return item.GetName();
        }
        private static string GetItemDescription(Loot item)
        {
            PowerUp x = item as PowerUp;
            if(x!=null)
                return x.Description.ToString();
            return string.Empty;
        }
        private static Sprite GetItemIcon(Loot item)
        {
            return item.sprite;
        }
        #endregion
